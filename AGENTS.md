...
`llmwiki` is a Node.js and TypeScript knowledge compiler. It turns raw sources
into interlinked, citation-traceable Markdown knowledge that can be used through
the CLI, SDK, MCP server, exports, and local viewer.

This repository is a personal fork of the general-purpose upstream project. New
work must remain easy to synchronize with upstream: prefer profiles,
configuration, isolated modules, and small extension points over invasive core
changes. The first fork-specific product direction is defined in
...
This is an npm package with a CLI and library bundle, not a long-running web
application. Use Node.js 24 and the committed npm lockfile.

- Install: `npm ci`
- Development build watcher: `npm run dev`
- Viewer development runner: `npm run dev:view`
- Typecheck: `npx tsc --noEmit`
- Build: `npm run build`
- Test: `npm test`
- Test watch mode: `npm run test:watch`
- Packaging test: `npm run test:pack`
- Codebase health: `npm run fallow:ci`
- Release documentation check: `npm run release:check-docs`
- Current-version release check: `npm run release:check-docs:current`

There is no single combined `Verify` command. Before a task is complete, run
Typecheck, Build, Test, and Codebase health in that order. Every check must pass,
and unresolved Fallow findings block completion.

The existing viewer tests use Vitest, jsdom, and live HTTP subprocess coverage.
No separate browser-test command is configured.
